...
2. Featureブランチで実装し、Vercel Previewは通常確認に使わない。ただしM1.5は例外としてcandidateと同一SHAのPreviewを作り、ユーザー実iPhone承認をmain前に必須とする。
...
10. 必須のmain前gateを満たした後だけマージし、Vercel、Production Smoke、Production Browser Smoke、Visual Evidenceを確認する。M1.5ではPreview承認後にコード・素材が変われば新SHAで再承認する。
