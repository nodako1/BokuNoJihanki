# 開発ロードマップ

## M0 開発基盤 — 完了

React＋Phaser＋Vite、横画面、PWAマニフェスト、タイトル、時間帯補間、Web Audio、ビルド表示、CI、Vercel。

## M1 町を歩く基盤 — ランタイム修正・Production再確認中

4方向移動、仮想スティック、キーボード、カメラ、住宅街・公園、4チャンクストリーミング、衝突、Yソート、時間帯、環境音、HUD、オリジナルSVG。

- 初回M1機能マージ: `432108024b40c9f3fc20aeec38b7bf871c192da1`
- 完了扱いを取り消したProductionコミット: `a169757a823e4ad19205072ab3ea1fc8651547aa`
- 判明した障害: PhaserのSVG Data URI復号例外により、開始後のCanvasが黒画面・0 FPSになる
- 黒画面修正マージ: `28b7ab6454d523d8ba4c4572e5c940356d8a5513`
- 修正: UTF-8 SVGをbase64 Data URIとして読み込む
- 再発防止: PRとmainの`Browser Smoke`で実ブラウザーの開始、FPS、座標、チャンク、移動、pageerrorを検査
- 現在: Vercel Hobbyのbuild rate limit解除後にProduction再試行予定

M1は、修正コミットがVercelへ反映され、Productionで開始、描画、移動が実ブラウザー確認できた時点で再び完了とします。

## M2 自販機探索と経済 — M1再確認後

自販機接近、状況対応ボタン、下／返却口、接写画面、固定乱数、空振りから1000円、所持金、15分経過、当日一回、18時制限、一時セーブ。

M2の完了条件は「自販機を見つけ、調べ、結果を確認し、15分と所持金が動く」一連の中心体験がProductionで動作することです。

## M3 1日の完成

自室6時、18時帰宅、日付変更、日記、ゲームショップ、1日3本、購入、複数日、再開。

## M4 中央・南エリア

駅前、商店街、学校、市民プール、ビーチ、プールセット、入場料、エリア期待値。

## M5 北エリアと移動

神社、山、遊園地、外周ショートカット、バス、穴掘りセット、探知機、埋蔵金。

## M6 NPC・会話・イベント

主要人物、上半身会話、スケジュール、フラグ、お手伝い、プレゼント、お礼、勝負。

## M7 31日

8月1日〜31日、週末イベント、2日間夏祭り、21時延長、都市伝説、在庫93本。

## M8 本番アート・音統一

背景・人物・表情・BGM・環境音・効果音の情報量と一貫性を仕上げる。

## M9 日記・エンディング

31日分の日記、ページめくり、ソフト本数、S〜Dランク。

## M10 正式版

経済シミュレーション、端末検証、性能、PWA更新、セーブ互換、v1.0.0。
