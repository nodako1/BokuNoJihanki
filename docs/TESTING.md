...
## M1.4 2D横スクロール（Release Candidate実装済み・Production確認前）

### navigation単体テスト

PR #33でmainへ取り込まれたnavigationコアについて、area graph、exit、spawn、横移動、9状態の遷移state machine、input lock、データ検証をPhaserなしで確認する。`tests/m14-area-graph.test.mjs`、`tests/m14-horizontal-movement.test.mjs`、`tests/m14-area-transition.test.mjs`、`tests/m14-navigation-state.test.mjs`がcoreを直接検証する。

`tests/m14-integration.test.mjs`と`tests/project-structure.test.mjs`では、`src/game/navigationAdapter/m14NavigationAdapter.mjs`がfallbackではなくcoreを直接import・呼び出すこと、`horizontalAxis`を`resolveHorizontalMovement`へ直接渡すこと、coreの9状態をScene公開層の4状態へ変換することを確認する。
...
- 単純なblurで入力、速度、足音が停止するが、不要なmaster gain変更は行わない
