...
この状態は、実装、PR検証、mainへのマージ、Vercel Production反映、Production Browser Smoke、Production実画面確認のすべてが完了するまで変更しない。Vercelのデプロイが`Ready`になっただけでは完了としない。
...
M1.3のコードとアセットはProductionフォールバックおよび設計履歴として残す。M2の`src/game/economy/`コアも変更・削除せず保存する。M1.4がProduction確認済みになるまでM2のScene統合へ進まない。
...
> Release Candidate実装注記: `claude/m1-4-area-navigation-core`が未到着のため、現時点では`src/game/navigationAdapter/`のPhaser非依存fallbackが同じ契約を提供する。Claude core統合とClaude本人のレビューは、branch／PR到着まで未完了として追跡し、完了済みとは記録しない。
...
Claudeの`src/game/navigation/`は純粋ロジックとして扱い、PhaserのScene、Sprite、Camera、Texture、Tween、React DOM、Web Audioを参照させない。

ChatGPT側のadapterは次だけを担当する。
...
接地フレーム2、7で左右足音を交互に鳴らす。実座標が変わらない場合は鳴らさない。エリア切り替え時は旧エリアmixを短く下げ、新エリアmixをフェードインする。AudioContextは最初のユーザー操作で開始し、ミュート状態を遷移後も維持する。非表示タブでは停止または十分に減衰し、復帰時に意図せず足音を連打しない。
...
M1.4がProduction確認済みになるまでは、この仕様書を「実装中・Production確認前」のままとし、未完了の検証を完了済みと記録しない。
