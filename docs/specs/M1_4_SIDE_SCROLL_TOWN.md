...
**Release Candidate実装済み・Production確認前**

M1.4のコード、アセット、navigation core統合は実装済みである。PR最終gate、mainへのマージ、Vercel Production反映、Production Smoke、Production Browser Smoke、Production実画面確認のすべてが完了するまでProduction確認済みへ変更しない。Vercelのデプロイが`Ready`になっただけでは完了としない。M1全体の最終判定と証跡欄は[M1完了計画](M1_COMPLETION_PLAN.md)を正とする。

## 1. 決定と目的

M1.4から、街探索のメイン方式を斜め見下ろし2.5D自由移動から、高解像度2D横スクロールへ正式に切り替えた。
...
M1.3のコードとアセットはrollback用fallbackおよび設計履歴として残す。M2の`src/game/economy/`コアも変更・削除せず保存し、M1.4画面からは接続しない。M1.4がProduction確認済みになるまでM2のScene統合へ進まない。
...
- Claudeのnavigationコアを利用するadapter境界
- 統合テストとBrowser Smokeの実行基盤
- mainマージ後に実行するProduction Smoke、Production Browser Smoke、実画面確認のgate

> 実装注記: Claude navigation core PR #33はmainへマージ済み（`ee255a1a8413768d0e7dbdf512964268c8eaf276`）。`src/game/navigationAdapter/m14NavigationAdapter.mjs`はfallbackではなく`src/game/navigation/`のcoreを直接利用し、`horizontalAxis`を`resolveHorizontalMovement`へ直接渡す。coreの9状態はScene公開層の4状態へ変換する。
...
Navigation adapter
          │
          ▼
Claude navigation core（Phaser／React非依存）
  ├─ area graph
  ├─ 横移動の純粋ロジック
...
PR #33でmainへ取り込まれた`src/game/navigation/`は純粋ロジックとして扱い、PhaserのScene、Sprite、Camera、Texture、Tween、React DOM、Web Audioを参照させない。

`src/game/navigationAdapter/m14NavigationAdapter.mjs`はfallbackロジックを使用せず、coreのarea graph、horizontal movement、transition state、navigation stateを直接呼び出す。adapterは次を担当する。

- キーボード／タッチ入力をnavigationコアの入力形へ正規化する
- 正規化した`horizontalAxis`をcoreの`resolveHorizontalMovement`へ直接渡す
...
coreの遷移state machineは`idle`、`requested`、`fading-out`、`loading`、`spawning`、`fading-in`、`completed`、`cancelled`、`error`の9状態を持つ。adapterは遷移ロジックを複製せず、Scene公開層へ`idle`、`fading-out`、`loading`、`fading-in`の4状態として投影する。core状態はadapter内部で保持・必要時に復元し、Sceneはcore内部状態を所有しない。API不整合を発見した場合は、担当境界を確認してから修正する。
...
Claudeのnavigation単体テストに加え、M1.4統合テストで次を確認する。
...
Release Candidate実装後の完了判定順序は次のとおり。
...
10. PR実画面確認
11. Claudeによるnavigation境界と統合のレビュー
12. mainへマージ
...
- Claude navigationコアをadapter経由で使用する
- M1.3のrollback用fallback／設計履歴とM2 economyコアを保存している
- Quality、PR Browser Smoke、Claudeレビューが成功する
- mainへマージ済みである
...
M1.4がProduction確認済みになるまでは、この仕様書を「Release Candidate実装済み・Production確認前」のままとし、未完了の検証を完了済みと記録しない。
