...
このM1.4配信成功と既存証跡は維持する。その後の2026-07-23の[ユーザー実機所見](../evidence/M1_5_REAL_DEVICE_FINDINGS.md)で品質問題が確認されたため、M1全体の完成判定は再オープンした。現在の必須gateは[M1.5 実機品質修正](M1_5_REAL_DEVICE_POLISH.md)であり、M1.5完了、Production確認、ユーザー実機承認までM2を開始しない。
...
これらの`groundY`、spawn、triggerはM1.4実装時のランタイム基準値であり、背景道路との視覚的一致を保証する独立正解データではない。M1.5では背景上の道路面と分岐入口を先に特定し、開始、中央、終端、spawn、分岐で足裏差6 CSS px以内となるよう再測定する。
...
M1.5では「過度に」の主観判定を使わず、`1280×720`、`844×390`、`932×430`で主人公と遷移パネルの交差0、最短間隔12 CSS px以上、タッチ領域44×44 CSS px以上を測定する。
...
M1.4の当時の配信gateを満たしたため、M1.4は**Production確認済み**である。詳細は[Production Evidence](../evidence/M1_4_PRODUCTION_EVIDENCE.md)を正とする。この履歴はM1全体の現在の完成承認を意味しない。M1完成へ戻すには、M1.5受入条件、M1.5 Production確認、ユーザー実機承認が必要である。
