...
実装PR #32の最終head `5c6895d0d1e2ad31a95f6490e60cc26f89d290cf`をmain `147f770a4b73077c4e5dc0523839b3fefb789db4`としてマージし、Quality、PR Browser Smoke、Vercel、Production Smoke、Production Browser Smoke、公開画面の確認まで完了した。PR #34後の`29223ee31fd4fc4fbca21a37b01fe89277279647`は現在のmain／Production baselineである。`147f770...`はM1.4実装のProduction確認commit、`29223ee...`は最終文書・mobile gateを含むbaselineとして役割を分ける。M1.4の2D横スクロール方式をM1の正式基盤とする。

このM1.4配信成功と既存証跡は維持する。その後の2026-07-23の[ユーザー実機所見](../evidence/M1_5_REAL_DEVICE_FINDINGS.md)で品質問題が確認されたため、M1全体の完成判定は再オープンした。現在の必須gateは[M1.5 実機品質修正](M1_5_REAL_DEVICE_POLISH.md)であり、同一SHA Previewのユーザー実iPhone承認前はmainへ進めず、承認済みSHAのProduction確認までM2を開始しない。
...
M1.3のコードとアセットはフォールバックおよび設計履歴として残す。M2の`src/game/economy/`コアも変更・削除せず保存し、M1.4には接続しない。M1.4 Production確認時点ではM2のScene統合を次工程としていたが、後続の実機所見により現在の次工程は必須M1.5へ変更した。
...
これらの`groundY`、spawn、triggerはM1.4実装時のランタイム基準値であり、背景道路との視覚的一致を保証する独立正解データではない。M1.5では背景上の道路面と分岐入口を先に特定し、開始、中央、終端、spawn、分岐で表示上の足裏差を原則2 CSS px以内、各spawn設定点のYと独立groundの差を6 CSS px以内となるよう再測定する。
...
M1.5では「過度に」の主観判定を使わず、`1280×720`、`844×390`、`932×430`で主人公と遷移パネルの交差0、最短間隔12 CSS px以上、HUDとの交差0、タッチ領域44×44 CSS px以上を、上下 × trigger開始・中央・終端 × 左右向きの12状態で測定する。
...
M1.4時点のVisual Reviewと15画面Artifactは配信シナリオの履歴として有効である。ただし背景を独立正解にした足裏差、背景分岐とtriggerの一致、UIと主人公の交差、BGMの音楽品質は測定していなかった。後続の実機不合格に対する最終品質判定にはM1.5仕様を適用する。
...
M1.4の当時の配信gateを満たしたため、M1.4は**Production確認済み**である。詳細は[Production Evidence](../evidence/M1_4_PRODUCTION_EVIDENCE.md)を正とする。この履歴はM1全体の現在の完成承認を意味しない。M1完成へ戻すには、同一SHA Previewの自動テスト・独立QA・Evidence監査・ユーザー実iPhone承認、承認済みSHAのmainマージ、M1.5 Production再確認が必要である。
