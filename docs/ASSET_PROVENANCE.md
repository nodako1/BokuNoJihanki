...
M1.4は、既存ゲームの画像、キャラクター、背景、UI、音を複製せず、「ぼくの自販機」専用のオリジナル素材として制作した。参考資料から採用したのは、左右歩行、横スクロール、上下分岐矢印、短いエリア切り替えという抽象的な操作方式だけである。
...
3エリアは別々のmasterと別構図で実装し、同じ全景の左右反転・単純複製・リサイズによるエリア間流用をしていない。各masterからruntime panoramaを作る工程ではcrop後のリサイズを行う。住宅配置、道路形状、塀、電柱、標識、自転車、植木鉢、物置、木陰、自販機、前景をエリアごとに変えている。
...
`asset-manifest.json`へversion、revision、style、license、generator、pipeline、area寸法、groundY、player frame構成、全出力ファイルを記録済みである。生成物は実ゲームへ組み込み済みだが、3エリアが別構図に見えること、主人公の接地、時間帯差、スマートフォンでの可読性はPRとProductionのVisual Reviewで確認する。

### 音声

M1.4でも外部の音声サンプルは使用しない。セミ、鳥、風、遠い車、足音、遷移音、地名音、矢印音はWeb Audio APIで実行時生成するため、サンプル音声ファイルの来歴は発生しない。

M1.2、M1.3のアセットと生成工程は上書き・削除せず、fallbackと設計履歴として保存している。
