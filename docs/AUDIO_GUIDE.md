...
- 人が旋律、和音、反復構造を認識できる完成音源にする
- decode errorを0件にし、再生中のclippingを発生させない
- 最終出力のtrue peakを-1 dBTP以下にする
- ループ境界でクリック、急な無音、音量跳躍、拍の欠落がない自然なloopにする
- true peak、clipping、loop duration、ループ境界の客観解析結果を証跡へ残す
- 客観解析だけで合格にせず、ユーザー本人のiPhoneスピーカーで「音楽として成立している」という聴感承認を得る

M1.5開始時点で、完成BGM、客観解析、iPhone聴感承認は**未実施**である。詳細は[M1.5 必須実機品質修正仕様](specs/M1_5_REAL_DEVICE_POLISH.md)を正とし、Production確認とユーザー承認までM2を開始しない。
