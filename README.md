...
バージョン`0.1.0`、current milestoneは **M1.5 必須実機品質修正（実装進行中）**です。

M1.4で街探索の正式基盤を高解像度2D横スクロールへ切り替え、正常にProduction配信しました。その後のユーザー実iPhone確認で、主人公、3エリアの接地、上下導線、遷移パネル、BGMにM1 blockingの品質不足が確認されたため、M1全体の完成判定を再オープンしました。M1.3の住宅街実装とアセットはフォールバック／設計履歴として保存し、M2の経済コアも変更せず保持しています。M2 Scene統合とopen PR #31は停止中です。

### M1.4 Production確認履歴

- 実装Pull Request: `#32`
- 実装PR最終head: `5c6895d0d1e2ad31a95f6490e60cc26f89d290cf`
- M1.4実装のProduction確認commit: `147f770a4b73077c4e5dc0523839b3fefb789db4`
...
M1.4の配信成功履歴は取り消しません。ただし、既存invariantとスクリーンショット枚数だけでは後続の実機不合格項目を合格にできないため、現在のM1完成gateはM1.5です。

### M1.5 main進行gate

1. local candidateで自動テストと受入条件を満たす
2. local candidateと同一SHAのVercel Previewを作る
3. くーちゃんcandidate QAとリダ君Evidence監査を完了する
4. ユーザーがPreviewを実iPhoneで操作し、主人公、接地、上下導線、遷移パネル、BGMの5項目を承認する
5. 承認済みSHAだけをmainへマージする
6. Productionで対象SHA、Smoke、Browser Smoke、画面・音を再確認する

Preview承認後にコード・素材が変わった場合は、新SHAでPreview以降のgateをやり直します。ユーザー実iPhone承認前にmain／Productionへ進めず、Production確認までM2とPR #31を変更・マージしません。
...
M1.4は当時の配信手順をすべて完了し、Production確認済みです。現在のM1.5では、通常のfeature branch自動Preview停止方針に対する必須gateとして、candidateと同一SHAのVercel Previewを許可済みの手段で用意し、上記のPreview実iPhone承認後にだけmainへ進みます。詳細は[M1.5 必須実機品質修正](docs/specs/M1_5_REAL_DEVICE_POLISH.md)を正とします。
...
6. 必要なmain前gateを満たした後だけマージし、Vercel、Production Smoke、Production Browser Smoke、実画面を確認する。
7. モックアップを完成証跡として使わない。

詳細: [M1.4仕様](docs/specs/M1_4_SIDE_SCROLL_TOWN.md) / [M1.5必須実機品質修正](docs/specs/M1_5_REAL_DEVICE_POLISH.md) / [M1.3仕様](docs/specs/M1_3_RESIDENTIAL_VERTICAL_SLICE.md) / [アーキテクチャ](docs/ARCHITECTURE.md) / [開発ルール](docs/DEVELOPMENT_RULES.md) / [テスト](docs/TESTING.md) / [デプロイ](docs/DEPLOYMENT.md) / [ロードマップ](docs/ROADMAP.md)
