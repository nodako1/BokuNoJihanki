...
バージョン `0.1.0`、マイルストーン **M1**。主人公が住宅街から「なつかぜ公園」まで、画面切り替えなしで歩ける探索基盤を実装しています。Production確認前は`PROJECT_STATE.json`の`status`を完了扱いにしません。
...
Production Branchは`main`です。Feature系ブランチのPreviewデプロイは`vercel.json`で停止しています。mainへのマージ後、VercelのコミットSHAが機能マージコミットと一致し、Productionで住宅街を操作できることまで確認します。

## ロードマップ

- M0: React＋Phaser、横画面、PWA基盤、時間帯、音声、CI
- M1: 移動、住宅街・公園、ストリーミング、衝突、2.5D、時間帯連動
