...
原因は、コード内のオリジナルSVGをpercent-encoded Data URIとしてPhaserへ渡した際、Phaser 4.2.1のローダーがペイロードをbase64として復号し、`InvalidCharacterError`でScene起動前に停止していたことです。SVGをUTF-8対応のbase64 Data URIへ変換する修正を実装し、ローカル本番ビルドをChromiumで起動して、描画、FPS、初期チャンク、キーボード移動、住宅街から公園への到達まで検証するブラウザースモークを追加しています。
...
- 完了条件: 修正をmainへマージし、Vercelと実ブラウザーの`Production Smoke`が両方成功すること
...
- Chromiumで開始、描画、入力、ストリーミング、公園到達、JavaScript例外を検査する`Browser Smoke`
- Vercel Productionを実際に起動・操作して検査する`Production Smoke`
...
7. mainマージ後にVercel Productionと実ブラウザーの`Production Smoke`を確認する。
...
Production Branchは`main`です。Feature系ブランチのPreviewデプロイは`vercel.json`で停止しています。機能コードをmainへマージすると、Vercel成功後にPlaywright ChromiumでProductionを開き、「夏休みを始める」を押し、FPSとチャンク読み込みを確認し、矢印キーで住宅街から公園まで移動します。ページ例外、主要リソースの読み込み失敗、0 FPS、座標未初期化、黒いCanvas、期待コミット不一致のいずれかがあれば`Production Smoke`を失敗させます。
...
8. Vercel Productionの対象コミットと実ブラウザーの`Production Smoke`
