...
const M15_RUNTIME_FILES = [
  ...
  'tests/m15-playwright-native-visibility.test.mjs',
...
for (const marker of [
  ...
  "status === 'patched'",
  "createPolicyReport(policy, 'already-patched')",
  "flag: 'wx'",
  ...
  if (!playwrightNativeVisibility.includes(marker)) {
    failures.push(
      `M1.5 Playwright native-visibility preparation is missing ${marker}.`,
    );
  }
}
for (const marker of [
  'M15_PLAYWRIGHT_NATIVE_VISIBILITY_POLICY',
  'replaces exactly one forced-active call and verifies hashes',
  'rejects an altered Playwright target even if the call remains',
  'rejects duplicate or missing focus-emulation calls',
  'rejects Playwright version and resolved-path mismatches',
  'accepts only the exact patched source on rerun',
  'writes a verified policy artifact without touching node_modules',
]) {
  if (!playwrightNativeVisibilityTest.includes(marker)) {
    failures.push(
      `M1.5 Playwright native-visibility contract test is missing ${marker}.`,
    );
  }
}
...
for (const marker of [
  ...
  'validate_browser_lifecycle_launch_policy',
  ...
  'browser_process.get("executablePath") == browser_executable',
  'browser_process.get("executableBytes") == browser_executable_bytes',
  'browser_process.get("executableSha256") == browser_executable_sha256',
  'browser_binary.get("executableBytes") == browser_executable_bytes',
  'browser_binary.get("executableSha256") == browser_executable_sha256',
  'value["playwrightNativeVisibility"]',
  '"Playwright forced-active focus emulation" in value["reason"]',
  ...
  'browser_process.get("pid") == browser_pid',
  'browser_process == runtime_browser_process',
  'Path(browser_process["executablePath"]).is_absolute()',
  'browser_process.get("executableBytes") == M15_GOOGLE_CHROME_ELF_BYTES',
  'browser_process["executableSha256"]',
...
for (const marker of [
  ...
  "addTamper('browser-process-hash'",
  "addTamper('browser-process-bytes'",
  "addTamper('wm-class'",
  "addTamper('arbitrary-profile-class'",
...
for (const marker of [
  ...
  'real X11 tab cycle restores native visible output',
  'domEventObserved',
  'headlessConstraint',
  'relatedUnitTest',
...
for (const marker of [
  ...
  'M15_GOOGLE_CHROME_VERSION: 150.0.7871.186',
  'M15_GOOGLE_CHROME_PACKAGE_VERSION: 150.0.7871.186-1',
  'M15_GOOGLE_CHROME_DEB_URL: https://dl.google.com/linux/chrome/deb/pool/main/g/google-chrome-stable/google-chrome-stable_150.0.7871.186-1_amd64.deb',
  "M15_GOOGLE_CHROME_DEB_BYTES: '133561688'",
  'M15_GOOGLE_CHROME_DEB_SHA256: 4193e00b6d5d5969ee63f7a69596868f546aa0e8cb077b3e0bf9cc1e2c719d00',
  "M15_GOOGLE_CHROME_ELF_BYTES: '280960248'",
  'M15_GOOGLE_CHROME_ELF_SHA256: 47e00a55c9e412ccb3b5a128fdf3b34378faecb0190b293829ddee28c6d8659e',
  ...
  'playwright-native-visibility-install.json',
  'test -x node_modules/.bin/playwright',
  './node_modules/.bin/playwright install --with-deps chromium',
  '--allow-downgrades',
  'CHROME_DEB_PATH="$RUNNER_TEMP/google-chrome-stable.deb"',
  "curl --fail --location --silent --show-error \\",
  "--proto '=https'",
  '--retry-all-errors',
  "stat -c '%s' \"$CHROME_DEB_PATH\"",
  '"$M15_GOOGLE_CHROME_DEB_BYTES"',
  '"$M15_GOOGLE_CHROME_DEB_SHA256"',
  'sha256sum --check --strict -',
  'dpkg-deb --field "$CHROME_DEB_PATH" Package',
  'dpkg-deb --field "$CHROME_DEB_PATH" Version',
  'dpkg-deb --field "$CHROME_DEB_PATH" Architecture',
  '"$M15_GOOGLE_CHROME_DEB_URL"',
  '"$CHROME_DEB_PATH"',
  'fontconfig',
  'fonts-noto-cjk',
  'xdotool',
  ...
  "grep -Fx '/opt/google/chrome/chrome'",
  'test "${#CHROME_PATHS[@]}" -eq 1',
  'test "$(readlink -f "$CHROME_PATH")" = "$CHROME_PATH"',
  '"Google Chrome $M15_GOOGLE_CHROME_VERSION"',
  "stat -c '%s' \"$CHROME_PATH\"",
  '"$M15_GOOGLE_CHROME_ELF_BYTES"',
  '"$M15_GOOGLE_CHROME_ELF_SHA256"',
  'CHROME_SHA256="$M15_GOOGLE_CHROME_ELF_SHA256"',
  'echo "BROWSER_EXECUTABLE_PATH=$CHROME_PATH" >> "$GITHUB_ENV"',
  'echo "BROWSER_EXECUTABLE_SHA256=$CHROME_SHA256" >> "$GITHUB_ENV"',
...
for (const marker of [
  ...
  'fontconfig fonts-noto-cjk openbox xdotool x11-utils xvfb',
  'node scripts/prepare-playwright-native-visibility.mjs',
  'playwright-native-visibility.json',
  'M15_GOOGLE_CHROME_VERSION',
  'M15_GOOGLE_CHROME_PACKAGE_VERSION',
  'M15_GOOGLE_CHROME_ELF_BYTES',
  'M15_GOOGLE_CHROME_ELF_SHA256',
  ...
  'sha256sum "$BROWSER_EXECUTABLE_PATH"',
  "dpkg-query --show --showformat='${Version}' google-chrome-stable",
  'browserExecutableSha256=%s',
  'playwrightNativeVisibilityPolicy=%s',
  'M15_RUNNER_OS_IMAGE',
...
if (/command -v google-chrome/.test(browserWorkflow)) {
  failures.push(
    'Browser Smoke must resolve the exact packaged Chrome ELF without PATH fallback.',
  );
}
