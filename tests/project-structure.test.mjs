...
test('M1.5 Browser Smoke and workflows enforce exact SHA and device contracts', async () => {
  const [
    ...
    buildBadge,
    buildTypes,
    playwrightNativeVisibilityPrepare,
    ...
    readFile('scripts/prepare-playwright-native-visibility.mjs', 'utf-8'),
    readFile('tests/m15-playwright-native-visibility.test.mjs', 'utf-8'),
  ]);
  ...
  for (const marker of [
    ...
    assert.ok(qualityWorkflow.includes(marker), marker);
  }
  assert.equal(
  ...
  for (const marker of [
    ...
    'observedSha256Before === policy.originalSha256',
    ...
    assert.ok(playwrightNativeVisibilityPrepare.includes(marker), marker);
  }
  for (const marker of [
    'M1.5 locks the exact Playwright 1.56.1 native visibility policy',
    'M1.5 replaces exactly one forced-active call and verifies hashes',
    'M1.5 rejects an altered Playwright target even if the call remains',
    'M1.5 rejects duplicate or missing focus-emulation calls',
    'M1.5 rejects Playwright version and resolved-path mismatches',
    'M1.5 accepts only the exact patched source on rerun',
    'M1.5 writes a verified policy artifact without touching node_modules',
    ...
    assert.ok(playwrightNativeVisibilityTest.includes(marker), marker);
  }
  for (const forbiddenPattern of [
    /writeFile\([^)]*crPage\.js/,
    /enabled:\s*false[^]*enabled:\s*true/,
  ]) {
    assert.doesNotMatch(playwrightNativeVisibilityPrepare, forbiddenPattern);
  }
  for (const marker of [
